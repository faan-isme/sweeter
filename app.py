...
def home():
    ...
    try:
        ...
        user_info = db.sweet.find_one({'username':payload['id']})
        return render_template("/menu/index.html",user_info=user_info)
...
def login():
    ...
    return render_template("/menu/login.html", msg=msg)
...
def user(username):
    ...
    try:
        ...
        user_info = db.sweet.find_one({"username": username}, {"_id": False})
        return render_template("/menu/user.html", user_info=user_info, status=status)
...
def check_dup():
    username_receive = request.form['username_give']
    exists = bool(db.sweet.find_one({"username": username_receive}))
...
def save_img():
    ...
    try:
        ...
        db.sweet.update_one({"username": payload["id"]}, {"$set": new_doc})
...
def get_posts():
    ...
    try:
        ...
        username_recive = request.args.get('username_give')
        if username_recive =='':
            posts = list(db.posts.find({}).sort("date", -1).limit(20))
        else:
            posts = list(db.posts.find({'username':username_recive}).sort("date", -1).limit(20))
        for post in posts:
